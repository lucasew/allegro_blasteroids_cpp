...
template <class T> void updateAll(T *o) { (*o).update(1); }

template <class T> void drawAll(T *o) {
  (*o).draw();
  std::cout << *o << std::endl;
}
...
class App {
  ...
  int checkCollision() {
    return checkCollisions(&this->bullets, &this->asteroids) +
           checkCollisionSpaceship(&this->ship, &this->asteroids);
  }
...
public:
  ...
  void spawnAsteroid() { this->asteroids.add(Asteroid(disp)); }
  void shotBullet() {
