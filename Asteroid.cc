...
const int asteroid_points[][2] = {{-20, 20},  {-25, 5}, {-25, -10}, {-5, -10},
                                  {-10, -20}, {5, -20}, {20, -10},  {20, -5},
                                  {0, 0},     {20, 10}, {10, 20},   {0, 15}};
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, const Asteroid &obj) {
    return out << "Asteroid(pos: " << obj.position << " scale: " << obj.scale
               << " heading: " << obj.heading << " speed: " << obj.speed
  ...
  void draw() {
    ...
    al_use_transform(&tr);
    for (int i = 0; i < 12; i++) {
      al_draw_line(asteroid_points[i][0] * this->scale,
                   asteroid_points[i][1] * this->scale,
                   asteroid_points[(i + 1) % 12][0] * this->scale,
                   asteroid_points[(i + 1) % 12][1] * this->scale, this->color,
                   1);
    }
  }
  float getDangerRadius() { return 22 * this->scale; }
};

bool isValid(Asteroid a) { return a.health > 0; }
