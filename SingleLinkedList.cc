...
#include <iostream>
#include <stdlib.h>
template <class T> struct genericSLL {
  T value;
  struct genericSLL *next;
};

template <class T> class SingleLinkedListIterator : public Iterator<T> {
  struct genericSLL<T> *cursor;
  int counter = 0;

public:
  SingleLinkedListIterator(struct genericSLL<T> *from) { this->cursor = from; }
  T *next() {
    if (cursor == NULL)
      throw counter;
    T *ret = &cursor->value;
...
public:
  ...
  ~SingleLinkedList<T>() { freeNodes(*this->root); }
  int getLength() { return length; }
  void add(T v) {
    struct genericSLL<T> *node =
        (struct genericSLL<T> *)malloc(sizeof(struct genericSLL<T>));
    node->value = v;
    node->next = *this->root;
    *this->root = node;
    length++;
  }
  // Checker é uma função que verifica se está certo com o nó, por exemplo um
  // asteroide com vida
